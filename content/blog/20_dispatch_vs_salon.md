...
「どっちが稼げるの？」という質問をよく受けますが、答えは「働き方による」です。

**出張型は、バック率が高めに設定されていることが多い。** お店が施術ルームの家賃を負担しない分、セラピストへの還元率を上げられるからです。ただし交通費が自己負担のお店だと、移動の多い日は手取りが目減りします。

**店舗型は、回転数で稼ぎやすい。** 移動時間がゼロなので、予約が埋まれば1日3〜4件を無理なくこなせます。[メンエスの日給目安が1.5万〜2.7万円（3接客）](/blog/09_menes_real)というのは、店舗型でコンスタントに稼働した場合のイメージに近いです。

つまり、**予約が安定して入るなら店舗型、稼働にムラがあるなら待機時間を自由に使える出張型**が、時間効率の面では合理的です。

もうひとつ現実的な話をすると、出張型は「移動も仕事のうち」です。施術60分＋移動往復60分なら、実質2時間で1件。時給換算すると見た目の日給より下がります。求人の数字を比較する時は、移動時間まで含めて考えてください。
